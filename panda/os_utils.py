...
def enable_ldap_admin(p_vip, local_user_name, local_user_pwd,
                      ldap_user_name):
...
def get_keystone_client(p_vip, username, password, project_name,
                        domain_name):
    sess = get_session(p_vip, username, password, project_name,
                       domain_name)
    return keystone_client.Client(session=sess)


def get_session(p_vip, username, password, project_name, domain_name):
    auth_url = get_auth_url(p_vip, 'v3', port='35357')
...
def get_auth_url(p_vip, version='v2.0', port='5000'):
    return 'https://%s:%s/%s/' % (p_vip, port, version)


def get_auth_url2(p_vip, version='v2.0', port='5000'):
    return 'https://%s:%s/%s' % (p_vip, port, version)
